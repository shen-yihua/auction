...
void auction(int array[RNUM][TNUM]);
...
void auction(int array[RNUM][TNUM])
{
	...
	double eplison = 1 / (double)(TNUM + 1);  //初始化放缩尺度
										   ...
										   //double bid[M] = { 0 };     //初始化竞价bid数组。
										   //int tmps1[M] = { 0 };
										   //int B[M] = { 0 };          //用来暂存竞标同一任务的机器人编号
	int s1[RNUM] = { -1,-1,-1,-1,-1 };					  //s1下标代表机器人，对应值为分配的任务
	int s2[TNUM] = { -1,-1,-1,-1,-1 };					  //s2下标代表任务，对应数组中的元素为被分配给的机器人
	int vi, wi, vij;			  //vi为最大利润，wi为次高利润，vij为计算出的当前利润
